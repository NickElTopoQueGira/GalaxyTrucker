...
public class Merce extends Tessera{
	...
	private final int massimo=24;
	private final TipoMerciGenerale tipoMerciGenerale;
	private int numeroMerciAttuale=0;
	...
	public void inserisciMerci(TipoMerce tipoSpecifico) {
		if(this.numeroMerciAttuale<this.MaxCapienza) {
			...
			if(this.tipoMerciGenerale==TipoMerciGenerale.NORMALI) {
				if(tipoSpecifico==TipoMerce.MERCE_GIALLA || tipoSpecifico==TipoMerce.MERCE_VERDE || tipoSpecifico==TipoMerce.MERCE_BLU) {
	...
	public void setValore(int edit) { //quando chiami questa funzione metterai nelle parentesi 
		//+ oppure - edit (ovvero: "nome variabile".getValore()) che serve. es: setValore(-tipoMerce1.getvalore()); PER AGGIUNGERE C'è LA FUNZIONE APPOSTA
